...
def _get_config(base_model="kontext", n_gpus=1, gradient_step_per_epoch=1, reward_fn={}, name=""):
    ...
    config.sample.num_image_per_prompt = 8 # 12
    ...
    config.run_name = f"motionnft_{base_model}_{name}"
    config.save_dir = f"{current_parent_directory}/checkpoints/motionnft_{base_model}/motionedit_8gpu_optical_flow_0to5_3e-4"
...
def kontext_motion_edit_reward_ban_prompt():
    reward_fn = {
        "motion_edit": 0.5,
        "mllm_score_continue": 0.5
    }
    config = _get_config(
        base_model="kontext",
        n_gpus=16, # 48
        ...
        reward_fn=reward_fn,
        name="motion_edit+mllm_score_continue",
    )
