...
# NEW: CROP BLACK BARS
def auto_crop_black_bars(img: Image.Image, 
                        var_threshold=5.0, 
                        edge_scan_frac=0.15):
    """
    Crop black bars (top/bottom or left/right) from an image.
    Returns: cropped_img, crop_direction, new_width, new_height

    crop_direction ∈ {"horizontal", "vertical", None}
    """

    gray = np.array(img.convert("L"))
    h, w = gray.shape

    # Only look at edges (where black bars appear)
    edge_h = int(h * edge_scan_frac)
    edge_w = int(w * edge_scan_frac)

    # Variance across rows (top/bottom detection)
    row_var = gray.var(axis=1)  # per row
    top_rows = np.argmax(row_var > var_threshold)
    bottom_rows = h - np.argmax((row_var > var_threshold)[::-1]) - 1

    # Variance across columns (left/right detection)
    col_var = gray.var(axis=0)  # per column
    left_cols = np.argmax(col_var > var_threshold)
    right_cols = w - np.argmax((col_var > var_threshold)[::-1]) - 1

    # Compute estimated black bar thickness
    vertical_bar_thickness = min(top_rows, h-bottom_rows)
    horizontal_bar_thickness = min(left_cols, w-right_cols)

    if vertical_bar_thickness > horizontal_bar_thickness:
        # Crop top & bottom → black bars horizontal
        crop_direction = "horizontal"
        cropped = img.crop((0, top_rows, w, bottom_rows+1))
    elif horizontal_bar_thickness > 0:
        # Crop left & right → black bars vertical
        crop_direction = "vertical"
        cropped = img.crop((left_cols, 0, right_cols+1, h))
    else:
        # No cropping needed
        return img, None, w, h

    new_w, new_h = cropped.size
    return cropped, crop_direction, new_w, new_h
...
class PromptImageDataset(Dataset):
    def __init__(self, dataset, resolution=512, split="train"):
        ...
        with open(self.file_path, "r", encoding="utf-8") as f:
            if self.split == 'test':
                self.metadatas = [json.loads(line) for line in f][:10]
                self.prompts = [item["prompt"] for item in self.metadatas][:10]
            else:
    ...
    def __getitem__(self, idx):
        ...
        image_paths = self.metadatas[idx]["image"]
        image_path, gt_image_path = image_paths[0], image_paths[1]
        ...
        gt_image = Image.open(os.path.join(self.dataset, gt_image_path)).convert("RGB")
        ...
        if crop_direction is not None:
            ...
            assert pred_img_w == pred_img_w
            assert pred_img_h == pred_img_h
        ...
        if w != h:
            ...
            right = left + min_dim
            bottom = top + min_dim
            gt_image = gt_image.crop((left, top, right, bottom))
...
def eval_fn(
    ...
    for test_batch in tqdm(
        ...
        prompts, prompt_metadata, ref_images, gt_images = test_batch
        with torch_autocast(
        ...
        rewards_future = executor.submit(
            reward_fn, images, prompts, prompt_metadata, ref_images, gt_images, only_strict=False
        )
        time.sleep(0)
...
def main(_):
    ...
    for epoch in range(first_epoch, config.num_epochs):
        ...
        for i in tqdm(
            ...
            prompts, prompt_metadata, ref_images, gt_images = next(
                train_iter
            )
