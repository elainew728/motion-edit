...
from .utils_dtype import to_like, grid_sample_dtype_safe  # add this import

# --- utils: safe grid_sample that matches dtype/device and falls back cleanly ---

def safe_grid_sample(img, grid, mode='bilinear', padding_mode='zeros', align_corners=True):
    # Make grid match img (this fixes "expected … BFloat16 but found Float")
    if grid.device != img.device:
        grid = grid.to(img.device)
    if grid.dtype != img.dtype:
        grid = grid.to(img.dtype)

    try:
        return F.grid_sample(img, grid, mode=mode, padding_mode=padding_mode, align_corners=align_corners)
    except RuntimeError as e:
        # Some wheels/hardware don’t support bf16 grid_sample; upcast then cast back.
        if img.dtype == torch.bfloat16:
            out = F.grid_sample(img.float(), grid.float(), mode=mode,
...
def global_correlation_softmax(feature0, feature1,
    ...
    # --- FIX: Cast the grid to match the feature's dtype ---
    grid = grid.to(feature0.dtype)
...
def local_correlation_softmax(feature0, feature1, local_radius, padding_mode='zeros'):
    b, c, h, w = feature0.size()
    # unify device/dtype from feature1 (the sampled tensor)
    dev, dt = feature1.device, feature1.dtype

    # coords in pixel space
    coords_init = coords_grid(b, h, w).to(device=dev, dtype=dt)           # [B, 2, H, W]
    coords = coords_init.view(b, 2, -1).permute(0, 2, 1)                  # [B, H*W, 2]
    ...
    window_grid = generate_window_grid(
        ...
        local_h, local_w, device=dev
    ).to(dtype=dt)                                                         # [2R+1, 2R+1, 2]

    window_grid = window_grid.reshape(-1, 2).repeat(b, 1, 1, 1)           # [B, 1, (2R+1)^2, 2]
    sample_coords = (coords.unsqueeze(-2) + window_grid).to(dtype=dt)     # [B, H*W, (2R+1)^2, 2]
    sample_coords_softmax = sample_coords                                  # keep pixel-space dtype consistent

    # mask out-of-bounds (OK in float/bf16)
    valid_x = (sample_coords[..., 0] >= 0) & (sample_coords[..., 0] < w)   # [B, H*W, (2R+1)^2]
    valid_y = (sample_coords[..., 1] >= 0) & (sample_coords[..., 1] < h)
    valid = valid_x & valid_y

    # normalize coords to [-1, 1] and match dtype/device to feature1 for grid_sample
    sample_coords_norm = normalize_coords(sample_coords, h, w).to(device=dev, dtype=dt)  # [B, H*W, (2R+1)^2, 2]

    # grid_sample expects: input [B,C,H,W], grid [B, Hout, Wout, 2]
    # some builds lack bf16 support; fall back to fp32 if needed
    try:
        window_feature = F.grid_sample(
            feature1, sample_coords_norm, mode='bilinear',
            padding_mode=padding_mode, align_corners=True
        )
    except RuntimeError as e:
        if dt == torch.bfloat16 and "expected scalar type" in str(e):
            window_feature = F.grid_sample(
                feature1.float(), sample_coords_norm.float(), mode='bilinear',
                padding_mode=padding_mode, align_corners=True
            ).to(dtype=dt)
        else:
            raise
    ...
    # -> [B, H*W, C, (2R+1)^2]
    window_feature = window_feature.permute(0, 2, 1, 3)

    # [B, H*W, 1, C]
    feature0_view = feature0.permute(0, 2, 3, 1).reshape(b, h * w, 1, c).to(dtype=dt)

    # correlation with dtype-safe scaling
    corr = torch.matmul(feature0_view, window_feature).view(b, h * w, -1)  # [B, H*W, (2R+1)^2]
    scale = torch.sqrt(torch.tensor(float(c), device=dev, dtype=dt))
    corr = corr / scale

    # mask invalid locations before softmax
    corr = corr.masked_fill(~valid, torch.finfo(dt).min if dt.is_floating_point else -1e9)

    prob = F.softmax(corr, dim=-1)                                        # [B, H*W, (2R+1)^2]

    # probability-weighted coordinates -> correspondence (keep dtypes aligned)
    correspondence = torch.matmul(
        prob.unsqueeze(-2).to(dtype=dt),                                  # [B, H*W, 1, (2R+1)^2]
        sample_coords_softmax.to(dtype=dt)                                 # [B, H*W, (2R+1)^2, 2]
    ).squeeze(-2).view(b, h, w, 2).permute(0, 3, 1, 2)                    # [B, 2, H, W]

    flow = correspondence - coords_init                                    # [B, 2, H, W] (dt)
    match_prob = prob                                                      # [B, H*W, (2R+1)^2] (dt)
...
def local_correlation_with_flow(feature0, feature1,
    ...
    # unify dtype/device across all inputs that participate in matmul/grid_sample
    feature0 = feature0.to(dev, dt)
    feature1 = feature1.to(dev, dt)
    if not isinstance(flow, float):
        flow = flow.to(dev, dt)

    # base grid (pixel coords) in the SAME dtype/device as feature tensors
    coords_init = coords_grid(b, h, w).to(dev, dt)                  # [B, 2, H, W]
    coords_flat = coords_init.view(b, 2, -1).permute(0, 2, 1)       # [B, H*W, 2]
    ...
    window_grid = generate_window_grid(-local_radius, local_radius,
                                       -local_radius, local_radius,
                                       local_h, local_w, device=dev).to(dt)  # [2R+1, 2R+1, 2]
    window_grid = window_grid.reshape(-1, 2).repeat(b, 1, 1, 1)     # [B, 1, (2R+1)^2, 2]

    sample_coords = coords_flat.unsqueeze(-2) + window_grid * dilation  # [B, H*W, (2R+1)^2, 2]
    if not isinstance(flow, float):
        sample_coords = sample_coords + flow.view(b, 2, -1).permute(0, 2, 1).unsqueeze(-2)

    # grid_sample expects coords normalized to [-1,1] and same dtype/device as img
    sample_coords_norm = normalize_coords(sample_coords, h, w).to(dev, dt)

    # sample windowed features with dtype-safe call
    window_feature = safe_grid_sample(
        feature1, sample_coords_norm, padding_mode=padding_mode, align_corners=True
    ).permute(0, 2, 1, 3)                                            # [B, H*W, C, (2R+1)^2]

    # query features for correlation
    feature0_view = feature0.permute(0, 2, 3, 1).reshape(b, h * w, 1, c).to(dt)
