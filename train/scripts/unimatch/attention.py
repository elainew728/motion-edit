...
class SelfAttnPropagation(nn.Module):
    ...
    def forward(self, feature0, flow,
        ...
        # --- FIX: Perform scaling without changing the data type ---
        # 1. Create a bfloat16 tensor from the high-precision scalar.
        scaling_factor = torch.tensor(c ** 0.5, dtype=query.dtype, device=query.device)

        # By casting the divisor to the same dtype, we prevent upcasting.
    ...
    def forward_local_window_attn(self, feature0, flow,
        ...
        flow_window = flow_window.to(dtype=feature0_reshape.dtype)
