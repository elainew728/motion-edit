...
class UniMatch(nn.Module):
    ...
    def forward(self, img0, img1,
        ...
        # ...
        if task == 'flow':
            # Store the original dtype (e.g., torch.bfloat16)
            original_dtype = img0.dtype
            original_device = img0.device

            # Normalize the images (this likely converts them to float32)
            img0, img1 = normalize_img(img0, img1)

            # --- FIX: Cast them back to the original dtype ---
            img0 = img0.to(device=original_device, dtype=original_dtype)
            img1 = img1.to(device=original_device, dtype=original_dtype)
        ...
        for scale_idx in range(self.num_scales):
            ...
            # --- AFTER ---
            # Inside the for-loop of the forward method

            # Store the original dtype before adding positional encodings
            original_dtype = feature0.dtype

            # add position to features (this likely converts them to float32)
            feature0, feature1 = feature_add_position(feature0, feature1, attn_splits, self.feature_channels)

            # FIX: Cast them back to the original dtype before the transformer
            feature0 = feature0.to(original_dtype)
            feature1 = feature1.to(original_dtype)
            ...
            if task == 'depth':
                ...
                feature0 = feature0.to(dtype=feature1.dtype, device=feature1.device)
