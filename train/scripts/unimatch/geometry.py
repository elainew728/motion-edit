...
def bilinear_sample(img, sample_coords, mode='bilinear', padding_mode='zeros', return_mask=False):
    # img: [B, C, H, W]
    # sample_coords: [B, 2, H, W] in image scale  (or [B, H, W, 2])

    # Ensure channel-first coords
    if sample_coords.size(1) != 2:  # [B, H, W, 2] -> [B, 2, H, W]
        sample_coords = sample_coords.permute(0, 3, 1, 2)

    # Match dtype/device to img (avoids Float vs BFloat16 mismatch)
    sample_coords = sample_coords.to(dtype=img.dtype, device=img.device)

    b, _, h, w = sample_coords.shape

    # Keep all constants/denominators in the SAME dtype/device to avoid upcasts
    two = torch.tensor(2.0, dtype=img.dtype, device=img.device)
    one = torch.tensor(1.0, dtype=img.dtype, device=img.device)
    w_denom = torch.tensor(max(w - 1, 1), dtype=img.dtype, device=img.device)
    h_denom = torch.tensor(max(h - 1, 1), dtype=img.dtype, device=img.device)

    # Normalize to [-1, 1]
    x_grid = two * sample_coords[:, 0] / w_denom - one
    y_grid = two * sample_coords[:, 1] / h_denom - one

    grid = torch.stack([x_grid, y_grid], dim=-1)  # [B, H, W, 2]

    # If your stack doesn’t support bf16 in grid_sample, fall back to f32 just for the op
    try:
...
def flow_warp(feature, flow, mask=False, padding_mode='zeros'):
    ...
    grid = grid.to(dtype=feature.dtype, device=feature.device)
