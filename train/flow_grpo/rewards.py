...
def motionedit_score(device):
    from flow_grpo.motionedit_scorer import MotionEditScorer

    scorer = MotionEditScorer(device=device)

    def _extract_single(item, name):
        if item is None:
            raise ValueError(f"{name} is required for motion edit scoring.")
        if isinstance(item, (list, tuple)):
            if len(item) != 1:
                raise ValueError(f"Motion edit scorer expects exactly one {name} entry per call.")
            return item[0]
        if isinstance(item, torch.Tensor) and item.ndim == 4:
            if item.shape[0] != 1:
                raise ValueError(f"Motion edit scorer expects batch size 1 for {name}.")
            return item[0]
        return item

    def _fn(images, prompts, metadata, ref_images=None, gt_images=None, **kwargs):
        if len(metadata) == 0:
            return np.zeros((0,), dtype=np.float32), {}

        pred_image = _extract_single(images, "image")
        ref_image = _extract_single(ref_images, "ref image")
        gt_image = _extract_single(gt_images, "ground truth image")

        score = scorer(pred_image, ref_image, gt_image)
        return score, {}
...
def multi_score(device, score_dict):
    ...
    def normalize_image_tensor(x: torch.Tensor):
        """
        Normalizes tensor shapes for images:
        - If shape is [3, H, W], return x unchanged.
        - If shape is [1, 3, H, W], return x[0] -> [3, H, W].
        - If shape is [B, 3, H, W] where B > 1, return list of B tensors, each [3, H, W].
        - Otherwise, raise an error.

        Returns:
        Tensor [3,H,W] or list of Tensors [3,H,W].
        """
        if x.ndim == 3:
            # Already [3, H, W]
            if x.shape[0] == 3:
            ...
            b, c, h, w = x.shape
            if c != 3:
                raise ValueError(f"Expected channels = 3, got {c}")

            if b == 1:
                # [1,3,H,W] -> [3,H,W]
                return [x[0]]
            else:
                # [B,3,H,W], B>1 -> list of [3,H,W]
    ...
    def _fn(images, prompts, metadata, ref_images=None, gt_images=None, only_strict=True):
        total_scores = []
        score_details = {}
        ...
        for score_name, weight in score_dict.items():
            ...
            elif score_name in ("image_similarity", "motion_edit"):
                # both need ref_images
                scores = [] # images.shape (1,3,512,512)
                images = normalize_image_tensor(images)
                for i in range(len(prompts)):
                    score, _ = score_fns[score_name]([images[i]], [prompts[i]], [metadata[i]], ref_images=[ref_images[i]], gt_images=[gt_images[i]])
                    scores.append(score)
            else:
